...
> **Limitations:**
> - **Shared services only.** Session-scoped services (`scoped="session"`) require a
>   persistent session connection that `aster call` doesn't establish. Use `aster shell`
>   or the proxy client for those.
> - **Unary methods only.** Streaming methods (`server_stream`, `client_stream`,
>   `bidi_stream`) need the shell or proxy client. `aster call` is request-response.
