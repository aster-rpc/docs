...
This service can be hosted with `AsterServer(services=[TaskService()])` and consumed via `AsterClient`. See [Hello Service](/examples/hello-service) for a runnable end-to-end example.
