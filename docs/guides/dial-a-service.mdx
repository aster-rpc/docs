...
See [AsterClient](/docs/bindings/python/client) for the full Python API reference.
...
The shell does not just discover methods -- it can invoke them too, without any
local type definitions. When connected to a peer, the shell reads contract
manifests and synthesizes wire-compatible types at runtime. Field names, types,
defaults, and Fory wire tags are all extracted from the manifest, so the shell
can build correctly serialized requests on the fly. This means you can explore
AND invoke any service from the shell without installing the service's client
package.
...
sequenceDiagram
    ...
    C->>P: QUIC connect (aster.consumer_admission)
    C->>P: credential (or empty for dev mode)
    P->>P: verify / auto-admit
    P-->>C: services list + registry ticket
    C->>P: QUIC connect (aster/1)
    C->>P: RPC calls
```
