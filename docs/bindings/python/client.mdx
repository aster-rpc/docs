...
AsterClient(
    ...
    peer="billing-consumer",               # optional: select peer from identity file
    address="aster1...",                    # producer's address
...
```python
# With identity file:
async with AsterClient(identity=".aster-identity", peer="billing-consumer") as c:
    svc = await c.client(MyService)
    ...
```

If `identity` is omitted, `AsterClient` auto-detects `.aster-identity` in the current directory. If `peer` is omitted, it auto-selects the first `[[peers]]` entry with `role = "consumer"`. If no identity file is found, the client runs in dev mode.
