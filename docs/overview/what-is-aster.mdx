...
**Serialization: Apache Fory.** The wire format is [Apache Fory](https://fury.apache.org), a high-performance cross-language serialization framework. Fory serializes native language objects directly -- no IDL compilation step, no generated code. It supports three modes: XLANG (cross-language, tagged types), NATIVE (single-language, maximum performance), and ROW (zero-copy random access).
...
The Python implementation is the reference binding. TypeScript is the second binding, sharing the same Rust core and wire protocol.

| Language | Status |
| --- | --- |
| Python | Reference implementation (active) |
| TypeScript | Alpha (active — mirrors Python feature set) |
| Rust | Planned |
| JVM (Java/Kotlin) | Planned |
| .NET (C#/F#) | Planned |
| Go | Planned |
