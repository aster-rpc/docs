...
class AnalyticsService {
  @Rpc({ request: Event, response: Ack }) // inherits XLANG from service
  async submitEvent(req: Event): Promise<Ack> { ... }

  @Rpc({ request: MetricsQuery, response: MetricsResult, serialization: SerializationMode.ROW }) // override to ROW
