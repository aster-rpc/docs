...
async with AsterServer(services=[HelloService()], identity=".aster-identity", peer="hello-producer") as srv:
    print(srv.address)
    await srv.serve()

# consumer.py -- loads consumer credential from .aster-identity
async with AsterClient(identity=".aster-identity", peer="hello-consumer") as c:
    hello = await c.client(HelloService)
    resp = await hello.say_hello(HelloRequest(name="World"))
```
