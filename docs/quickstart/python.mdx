...
# Enroll this node as a producer (hosts the billing service):
aster enroll node --role producer --name billing-producer
#   -> generates node key + signs credential -> writes .aster-identity

# Enroll the same node as a consumer (calls other services, e.g. orders):
aster enroll node --role consumer --name order-consumer
#   -> adds a consumer peer entry to the same .aster-identity
```

Each `aster enroll node` call adds a `[[peers]]` entry to the `.aster-identity` file. A single node can act as both producer and consumer -- for example, a billing service that also calls an order service.
...
async with AsterClient(peer="order-consumer") as c:
    hello = await c.client(HelloService)
    resp = await hello.say_hello(HelloRequest(name="World"))
```
...
:::note
For a deeper look at how identity files, peer entries, and multi-role enrollment work, see [Understanding the .aster-identity file](/docs/concepts/identity-file) *(coming soon)*.
:::
