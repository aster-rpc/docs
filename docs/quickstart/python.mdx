...
See [Configuration](/reference/configuration) for the full list of environment variables and TOML settings, and [Trust and admission](/concepts/trust-model) for the security model.

## What's next

- [Defining services and types](/guides/services) -- streaming RPCs, `@wire_type`, contract identity.
- [Configuration](/reference/configuration) -- `AsterConfig`, TOML files, environment variables.
- [Trust and admission](/concepts/trust-model) -- root keys, enrollment credentials, the Gate model.
- [AsterServer](/bindings/python/server) -- blobs, docs, gossip, Gate 0 wiring.
- [AsterClient](/bindings/python/client) -- admission flow, typed clients, error handling.
- [Observability](/guides/observability) -- structured logging, OTel metrics, health endpoints.
